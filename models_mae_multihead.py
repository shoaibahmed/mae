...
class MultiHeadMaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        assert depth % num_stages == 0, f"Depth: {depth} and number of stages: {num_stages} are not compatible!"
        self.num_blocks_per_stage = depth // num_stages
    ...
    def random_masking(self, x, mask_ratio_list):
        """
        Perform per-sample random masking by per-sample shuffling.
        Per-sample shuffling is done by argsort random noise.
        x: [N, L, D], sequence
        """
        assert isinstance(mask_ratio_list, list) or isinstance(mask_ratio_list, tuple), mask_ratio_list
        ...
        for mask_ratio in mask_ratio_list:
            ...
            # generate the binary mask for recons: 0 is keep, 1 is remove
    ...
    def forward_encoder(self, x, mask_ratio_list):
        # embed patches
        ...
        ids_keep_list, mask_list, ids_restore = self.random_masking(x, mask_ratio_list)
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i % self.num_blocks_per_stage == self.num_blocks_per_stage - 1:  # Stop gradient and concatenate it with features
                # Concatenate the outputs to the list (wihtout stopping the gradient)
                block_idx = len(encoded_features_list)
    ...
    def forward(self, imgs, mask_ratio_list=[0.2, 0.4, 0.6, 0.75]):
        ...
        return loss_list, pred_list, mask_list


def multihead_mae_vit_base_patch16_dec512d8b(**kwargs):
    model = MultiHeadMaskedAutoencoderViT(
...
def multihead_mae_vit_large_patch16_dec512d8b(**kwargs):
    model = MultiHeadMaskedAutoencoderViT(
...
def multihead_mae_vit_huge_patch14_dec512d8b(**kwargs):
    model = MultiHeadMaskedAutoencoderViT(
...
multihead_mae_vit_base_patch16 = multihead_mae_vit_base_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
multihead_mae_vit_large_patch16 = multihead_mae_vit_large_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
multihead_mae_vit_huge_patch14 = multihead_mae_vit_huge_patch14_dec512d8b  # decoder: 512 dim, 8 blocks
